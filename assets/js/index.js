...
const propiedadesJSON = [
  ...
  },
  {
    nombre: "Ley Pereira",
    descripcion:
      "Para los que gustan de un estilo funcional, con  buen gusto y un toque inglés",
...
function buscarPropiedad() {
  ...
  if (cuartos == 0 || desde == 0 || hasta == 0) {
    ...
    propSeleccionada = propiedadesJSON.filter((propiedadSel) => {
      return (
        propiedadSel.cuartos >= cuartos &&
        propiedadSel.metros >= desde &&
        propiedadSel.metros <= hasta
      );
    });

    for (let propiedadFiltrada of propSeleccionada) {
      template(propiedadFiltrada);
    }
    console.log("propSeleccionada->", propSeleccionada.length);

    total = propSeleccionada.length;
    cantidad.innerHTML = "Propiedades encontradas: " + total;
    console.log(total);
  }
  ...
  if (total == 0) {
    ...
    alert("No hay propiedades que cumplan con los criterios de busqueda");
  }
}
